							   ...
							   const QModelIndex &index) : QMainWindow(), biblioteq_item(index)
{
	...
	id.binding->addItems(biblioteq_misc_functions::getBookBindingTypes(qmain->getDB(),
																	   errorstr));
	...
	QApplication::setOverrideCursor(Qt::WaitCursor);
	id.language->addItems(biblioteq_misc_functions::getLanguages(qmain->getDB(),
																 errorstr));
	QApplication::restoreOverrideCursor();
	...
	QApplication::setOverrideCursor(Qt::WaitCursor);
	id.location->addItems(biblioteq_misc_functions::getLocations(qmain->getDB(),
																 "Book",
																 errorstr));
	QApplication::restoreOverrideCursor();
...
biblioteq_item_working_dialog *biblioteq_book::createImageDownloadDialog(const QString &downloadType)
{
	biblioteq_item_working_dialog *dialog = nullptr;

	dialog = new biblioteq_item_working_dialog(qobject_cast<QMainWindow *>(this));
...
void biblioteq_book::createOpenLibraryDialog(void)
{
	...
	m_openLibraryWorking = new biblioteq_item_working_dialog(qobject_cast<QMainWindow *>(this));
...
void biblioteq_book::createSRUDialog(void)
{
	...
	m_sruWorking = new biblioteq_item_working_dialog(qobject_cast<QMainWindow *>(this));
...
void biblioteq_book::downloadFinished(void)
{
	if (m_imageBuffer.property("which") == "front")
	{
		...
		{
			m_imageBuffer.close();
			QMessageBox::warning(this, tr("BiblioteQ: HTTP Warning"),
...
void biblioteq_book::insert(void)
{
	...
	id.url->clear();
	biblioteq_misc_functions::highlightWidget(id.id, QColor(255, 248, 220));
	biblioteq_misc_functions::highlightWidget(id.isbn13, QColor(255, 248, 220));
	biblioteq_misc_functions::highlightWidget(id.title, QColor(255, 248, 220));
	biblioteq_misc_functions::highlightWidget(id.publisher->viewport(), QColor(255, 248, 220));
	biblioteq_misc_functions::highlightWidget(id.place->viewport(), QColor(255, 248, 220));
	biblioteq_misc_functions::highlightWidget(id.author->viewport(), QColor(255, 248, 220));
	biblioteq_misc_functions::highlightWidget(id.description->viewport(), QColor(255, 248, 220));
	biblioteq_misc_functions::highlightWidget(id.category->viewport(), QColor(255, 248, 220));
	...
	else
	{
		...
		for (i = 0; i < record.count(); i++)
		{
			...
			if (fieldname == "title")
				...
				id.publication_date->setDate(QDate::fromString(var.toString().trimmed(), "MM/dd/yyyy"));
			else if (fieldname == "edition")
			{
				if (id.edition->findText(var.toString().trimmed()) > -1)
					id.edition->setCurrentIndex(id.edition->findText(var.toString().trimmed()));
				else
					id.edition->setCurrentIndex(0);
			}
			...
			else if (fieldname == "language")
			{
				if (id.language->findText(var.toString().trimmed()) > -1)
					id.language->setCurrentIndex(id.language->findText(var.toString().trimmed()));
				else
					id.language->setCurrentIndex(id.language->findText(biblioteq::s_unknown));
			}
			...
			else if (fieldname == "monetary_units")
			{
				if (id.monetary_units->findText(var.toString().trimmed()) > -1)
					id.monetary_units->setCurrentIndex(id.monetary_units->findText(var.toString().trimmed()));
				else
					id.monetary_units->setCurrentIndex(id.monetary_units->findText(biblioteq::s_unknown));
			}
			else if (fieldname == "binding_type")
			{
				if (id.binding->findText(var.toString().trimmed()) > -1)
					id.binding->setCurrentIndex(id.binding->findText(var.toString().trimmed()));
				else
					id.binding->setCurrentIndex(0);
			}
			else if (fieldname == "location")
			{
				if (id.location->findText(var.toString().trimmed()) > -1)
					id.location->setCurrentIndex(id.location->findText(var.toString().trimmed()));
				else
					id.location->setCurrentIndex(id.location->findText(biblioteq::s_unknown));
			}
			else if (fieldname == "id")
			{
				...
				id.isbn13->setText(qmain->formattedISBN13(var.toString().trimmed()));
			...
			else if (fieldname == "originality")
			{
				if (id.originality->findText(var.toString().trimmed()) > -1)
					id.originality->setCurrentIndex(id.originality->findText(var.toString().trimmed()));
				else
			...
			else if (fieldname == "condition")
			{
				if (id.condition->findText(var.toString().trimmed()) > -1)
					id.condition->setCurrentIndex(id.condition->findText(var.toString().trimmed()));
				else
			...
			else if (fieldname == "front_cover")
			{
				...
				{
					id.front_image->loadFromData(QByteArray::fromBase64(var.toByteArray()));
			...
			else if (fieldname == "back_cover")
			{
				if (!record.field(i).isNull())
				{
					id.back_image->loadFromData(QByteArray::fromBase64(var.toByteArray()));
...
void biblioteq_book::populateAfterOpenLibrary(void)
{
	...
	while (!keys.isEmpty())
	{
		...
		if (index > -1)
		{
					 ...
					 key == "\"title\":")
			{
				...
				{
					keys << key;
					m_openLibraryResults.remove(index - key.length(), key.length());
					continue;
				}
	...
	biblioteq_misc_functions::highlightWidget(id.marc_tags->viewport(), QColor(162, 205, 90));
	id.marc_tags->setPlainText(m_openLibraryResults);

	if (!authors.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.author->viewport(), QColor(162, 205, 90));
		id.author->setPlainText(authors);
	}

	if (!deweyDecimalClass.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.deweynum, QColor(162, 205, 90));
		id.deweynum->setText(deweyDecimalClass);
	}

	if (!lcClassifications.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.callnum, QColor(162, 205, 90));
		id.callnum->setText(lcClassifications);
	}

	if (!lccn.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.lcnum, QColor(162, 205, 90));
		id.lcnum->setText(lccn);
	}

	if (!pagination.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.description->viewport(), QColor(162, 205, 90));
		id.description->setPlainText(pagination);
	}

	if (!place.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.place->viewport(), QColor(162, 205, 90));
		id.place->setPlainText(place);
	}

	if (!publicationDate.isEmpty())
	{
		id.publication_date->setDate(QDate::fromString("01/01/" + publicationDate, "MM/dd/yyyy"));
		id.publication_date->setStyleSheet("background-color: rgb(162, 205, 90)");
	}

	if (!publishers.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.publisher->viewport(), QColor(162, 205, 90));
		id.publisher->setPlainText(publishers);
	}

	if (!subjects.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.category->viewport(), QColor(162, 205, 90));
		id.category->setPlainText(subjects);
	}

	if (!title.isEmpty())
	{
		biblioteq_misc_functions::highlightWidget(id.title, QColor(162, 205, 90));
...
void biblioteq_book::populateAfterSRU(const QString &recordSyntax, const QString &text)
{
	id.edition->setCurrentIndex(0);
	...
	while (!reader.atEnd())
		if (reader.readNextStartElement())
			...
			{
				auto tag(reader.attributes().value("tag").toString().toLower().trimmed());
	...
	if (recordSyntax == "MARC21")
		m.initialize(biblioteq_marc::BOOK, biblioteq_marc::SRU, biblioteq_marc::MARC21);
	else if (recordSyntax == "UNIMARC")
		m.initialize(biblioteq_marc::BOOK, biblioteq_marc::SRU, biblioteq_marc::UNIMARC);
	else
	{
		/*
		** Guess?
	...
	if (id.binding->findText(str, Qt::MatchFixedString) > -1)
		id.binding->setCurrentIndex(id.binding->findText(str, Qt::MatchFixedString));
	...
	if (!isbn13User && !str.isEmpty())
	{
		...
		biblioteq_misc_functions::highlightWidget(id.isbn13, QColor(162, 205, 90));
	}
...
void biblioteq_book::populateAfterZ3950(const QString &text, const biblioteq_marc::RECORD_SYNTAX recordSyntax)
{
	QString str("");
	QStringList list;
	biblioteq_marc m;
...
void biblioteq_book::populateFiles(void)
{
		...
		while (query.next() && totalRows < id.files->rowCount())
		{
			...
			for (int i = 0; i < record.count(); i++)
			{
				...
				if (record.fieldName(i) == "f_s")
					item = new biblioteq_filesize_table_item(locale.toString(query.value(i).toLongLong()));
				else
					item = new QTableWidgetItem(query.value(i).toString().trimmed());

				item->setData(Qt::UserRole, query.value(record.count() - 1).toLongLong());
	...
	id.files->horizontalHeader()->setSortIndicator(Columns::FILE, Qt::AscendingOrder);
...
{
	id.isbn13->setText(qmain->formattedISBN13(biblioteq_misc_functions::isbn10to13(id.id->text())));
}
...
void biblioteq_book::slotConvertISBN13to10(void)
{
	...
	id.id->setText(qmain->formattedISBN10(biblioteq_misc_functions::isbn13to10(id.isbn13->text())));
}
...
void biblioteq_book::slotDownloadImage(void)
{
	...
	{
		if (downloadType.contains("back"))
			url = QUrl::fromUserInput(qmain->getAmazonHash().value("back_cover_host") +
									  QString(qmain->getAmazonHash().value("back_cover_path")).replace("%", id.id->text().remove('-').trimmed()));
		else
			url = QUrl::fromUserInput(qmain->getAmazonHash().value("front_cover_host") +
									  QString(qmain->getAmazonHash().value("front_cover_path")).replace("%", id.id->text().remove('-').trimmed()));
		...
		else
		{
			if (type == "http" || type == "socks5" || type == "system")
			{
				...
				*/
				...
				auto reply = m_imageManager->get(QNetworkRequest(QUrl::fromUserInput("http://0.0.0.0")));
	...
	else
	{
			...
			id.id->text().remove('-').trimmed().length() == 10)
		{
			...
			string.replace("$value-$size", id.id->text().remove('-').trimmed() + "-L");
		}
		else
		{
			string.replace("$key", "olid");
			string.replace("$value-$size", id.alternate_id_1->text().trimmed() + "-L");
		}
...
void biblioteq_book::slotGo(void)
{
		...
		m_engWindowTitle.contains("Modify"))
	{
		if (m_engWindowTitle.contains("Modify") && m_index->isValid())
		{
			...
			QApplication::setOverrideCursor(Qt::WaitCursor);
			maxcopynumber = biblioteq_misc_functions::getMaxCopyNumber(qmain->getDB(), m_oid, "Book", errorstr);
		...
		if (id.multivolume_set_isbn->text().length() == 10)
			id.multivolume_set_isbn->setText(qmain->formattedISBN10(id.multivolume_set_isbn->text()));
		else if (id.multivolume_set_isbn->text().length() == 13)
			id.multivolume_set_isbn->setText(qmain->formattedISBN13(id.multivolume_set_isbn->text()));
			...
			*/
					 ...
					 qmain->getTypeFilterString() == "Books"))
				{
					...
					for (i = 0; i < names.size(); i++)
					{
						if (names.at(i) == "Call Number")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.callnum->text());
						else if (names.at(i) == "ISBN-10" ||
								 names.at(i) == "ID Number")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.id->text());
						else if (names.at(i) == "Title")
						{
							imageColumn = i;
							qmain->getUI().table->item(m_index->row(), i)->setText(id.title->text());
						}
						else if (names.at(i) == "Edition")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.edition->currentText().trimmed());
						else if (names.at(i) == "Authors")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.author->toPlainText());
						else if (names.at(i) == "Publication Date")
						{
							if (qmain->getTypeFilterString() == "Books")
								qmain->getUI().table->item(m_index->row(), i)->setText(id.publication_date->date().toString(qmain->publicationDateFormat("books")));
							else
								qmain->getUI().table->item(m_index->row(), i)->setText(id.publication_date->date().toString(Qt::ISODate));
						}
						else if (names.at(i) == "Publisher")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.publisher->toPlainText());
						else if (names.at(i) == "Place of Publication")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.place->toPlainText());
						else if (names.at(i) == "Categories")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.category->toPlainText());
						else if (names.at(i) == "Price")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.price->cleanText());
						else if (names.at(i) == "Language")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.language->currentText().trimmed());
						else if (names.at(i) == "Monetary Units")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.monetary_units->currentText().trimmed());
						else if (names.at(i) == "Quantity")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.quantity->text());
						else if (names.at(i) == "Book Binding Type")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.binding->currentText().trimmed());
						else if (names.at(i) == "Location")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.location->currentText().trimmed());
						else if (names.at(i) == "ISBN-13")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.isbn13->text());
						else if (names.at(i) == "LC Control Number")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.lcnum->text());
						else if (names.at(i) == "Call Number")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.callnum->text());
						else if (names.at(i) == "Dewey Class Number")
							qmain->getUI().table->item(m_index->row(), i)->setText(id.deweynum->text());
					...
					if (qmain->showMainTableImages())
					{
						...
						if (!pixmap.isNull())
							qmain->getUI().table->item(m_index->row(), imageColumn)->setIcon(pixmap);
						else
							qmain->getUI().table->item(m_index->row(), imageColumn)->setIcon(QIcon(":/no_image.png"));
					}
			...
			else
			{
				...
				if (qmain->getUI().actionAutoPopulateOnCreation->isChecked())
					(void)qmain->populateTable(biblioteq::POPULATE_ALL, "Books", QString(""));
	...
	else if (m_engWindowTitle.contains("Search"))
	{
		...
		searchstr.append(UNACCENT + "(LOWER(title)) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) AND ");
		...
		searchstr.append(UNACCENT + "(LOWER(COALESCE(callnumber, ''))) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) AND ");
		values.append(biblioteq_myqstring::escape(id.callnum->text().trimmed()));
		searchstr.append(UNACCENT + "(LOWER(COALESCE(deweynumber, ''))) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) AND ");
		...
		searchstr.append(UNACCENT + "(LOWER(author)) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) AND ");
		values.append(biblioteq_myqstring::escape(id.author->toPlainText().trimmed()));
		...
		searchstr.append(UNACCENT + "(LOWER(publisher)) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) AND ");
		values.append(biblioteq_myqstring::escape(id.publisher->toPlainText().trimmed()));
		searchstr.append(UNACCENT + "(LOWER(place)) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) AND ");
		values.append(biblioteq_myqstring::escape(id.place->toPlainText().trimmed()));
		searchstr.append(UNACCENT + "(LOWER(category)) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) AND ");
		values.append(biblioteq_myqstring::escape(id.category->toPlainText().trimmed()));
		...
		if (id.language->currentIndex() != 0)
		{
			...
			values.append(biblioteq_myqstring::escape(id.language->currentText().trimmed()));
		}

		if (id.monetary_units->currentIndex() != 0)
		{
			searchstr.append(UNACCENT + "(monetary_units) = " + UNACCENT + "(?) AND ");
		...
		{
			searchstr.append(UNACCENT + "(binding_type) = " + UNACCENT + "(?) AND ");
			values.append(biblioteq_myqstring::escape(id.binding->currentText().trimmed()));
		}

		searchstr.append(UNACCENT + "(LOWER(description)) LIKE " + UNACCENT +
						 "(LOWER(" + ESCAPE + "'%' || ? || '%')) ");
		values.append(biblioteq_myqstring::escape(id.description->toPlainText().trimmed()));

		if (id.quantity->value() != 0)
			searchstr.append("AND quantity = " + QString::number(id.quantity->value()) + " ");

		if (id.location->currentIndex() != 0)
		{
			searchstr.append("AND " + UNACCENT + "(location) = " + UNACCENT + "(?) ");
			values.append(biblioteq_myqstring::escape(id.location->currentText().trimmed()));
		}

		searchstr.append("AND " + UNACCENT + "(LOWER(COALESCE(marc_tags, ''))) LIKE " +
						 UNACCENT + "(LOWER(" + ESCAPE + "'%' || ? || '%')) ");
		values.append(biblioteq_myqstring::escape(id.marc_tags->toPlainText().trimmed()));
		searchstr.append("AND " + UNACCENT + "(LOWER(COALESCE(keyword, ''))) LIKE " +
						 UNACCENT + "(LOWER(" + ESCAPE + "'%' || ? || '%')) ");
		values.append(biblioteq_myqstring::escape(id.keyword->toPlainText().trimmed()));
		searchstr.append("AND " + UNACCENT + "(LOWER(COALESCE(url, ''))) LIKE " +
						 UNACCENT + "(LOWER(" + ESCAPE + "'%' || ? || '%')) ");
		values.append(biblioteq_myqstring::escape(id.url->toPlainText().trimmed()));

		if (id.originality->currentIndex() != 0)
		{
			searchstr.append("AND " + UNACCENT + "(originality) = " + UNACCENT + "(?) ");
			values.append(biblioteq_myqstring::
		...
		{
			searchstr.append("AND " + UNACCENT + "(condition) = " + UNACCENT + "(?) ");
			values.append(biblioteq_myqstring::
		...
		searchstr.append("LOWER(COALESCE(multivolume_set_isbn, '')) "
						 "LIKE LOWER('%' || ? || '%') ");
		...
		(void)qmain->populateTable(query, "Books", biblioteq::NEW_PAGE, biblioteq::POPULATE_SEARCH);
	}
}
	...
	else
	{
		if (type == "http" || type == "socks5" || type == "system")
		{
			...
			*/
			...
			auto reply = m_openLibraryManager->get(QNetworkRequest(QUrl::fromUserInput("http://0.0.0.0")));
...
void biblioteq_book::slotOpenLibraryQueryError(const QString &text)
{
	...
	QMessageBox::critical(this, tr("BiblioteQ: Open Library Query Error"),
...
void biblioteq_book::slotOpenLibrarySslErrors(const QList<QSslError> &list)
{
	...
	emit openLibraryQueryError(tr("One or more SSL errors occurred. Please verify your settings."));
}
...
void biblioteq_book::slotPopulateCopiesEditor(void)
{
	...
	copyeditor = new biblioteq_copy_editor_book(qobject_cast<QWidget *>(this),
												qmain,
	...
	if (action != nullptr)
	{
		...
		else if (action == actions[11])
		{
			if (m_engWindowTitle.contains("Search"))
			{
				...
				id.publication_date->setDate(QDate::fromString("01/01/2000", "MM/dd/yyyy"));
	...
	else
	{
		if (type == "http" || type == "socks5" || type == "system")
		{
			...
			*/
			...
			auto reply = m_sruManager->get(QNetworkRequest(QUrl::fromUserInput("http://0.0.0.0")));
...
void biblioteq_book::slotSRUQueryError(const QString &text)
{
	...
	QMessageBox::critical(this, tr("BiblioteQ: SRU Query Error"),
						  tr("A network error (%1) occurred.").arg(text.trimmed()));
	QApplication::processEvents();
}
...
void biblioteq_book::slotSRUSslErrors(const QList<QSslError> &list)
{
	...
	emit sruQueryError(tr("One or more SSL errors occurred. Please verify your settings."));
}
	...
	if (dialog.result() == QDialog::Accepted)
	{
		if (button == id.frontButton)
		{
			...
			if (dialog.selectedFiles().value(0).lastIndexOf(".") > -1)
				id.front_image->m_imageFormat = dialog.selectedFiles().value(0).mid(dialog.selectedFiles().value(0).lastIndexOf(".") + 1).toUpper();

			id.front_image->scene()->addPixmap(QPixmap::fromImage(id.front_image->m_image));

			if (!id.front_image->scene()->items().isEmpty())
				id.front_image->scene()->items().at(0)->setFlags(QGraphicsItem::ItemIsSelectable);

			id.front_image->scene()->setSceneRect(id.front_image->scene()->itemsBoundingRect());
		}
		else
		{
			id.back_image->clear();
			...
			if (dialog.selectedFiles().value(0).lastIndexOf(".") > -1)
				id.back_image->m_imageFormat = dialog.selectedFiles().value(0).mid(dialog.selectedFiles().value(0).lastIndexOf(".") + 1).toUpper();

			id.back_image->scene()->addPixmap(QPixmap::fromImage(id.back_image->m_image));

			if (!id.back_image->scene()->items().isEmpty())
				id.back_image->scene()->items().at(0)->setFlags(QGraphicsItem::ItemIsSelectable);

			id.back_image->scene()->setSceneRect(id.back_image->scene()->itemsBoundingRect());
		}
	}
}
...
void biblioteq_book::slotZ3950Query(void)
{
	...
	if (isbns.at(0).isEmpty())
		...
		searchstr = QString("@attr 1=7 @or %1 %2").arg(isbns.at(0)).arg(isbns.at(1));
	...
	for (i = 0; i < id.z3950QueryButton->actions().size(); i++)
		...
		{
			found = true;
			recordSyntax = qmain->getZ3950Hash(id.z3950QueryButton->actions().at(i)->text()).value("RecordSyntax");
			m_thread->setZ3950Name(id.z3950QueryButton->actions().at(i)->text());
			break;
		}

	if (!found)
	{
		recordSyntax = qmain->getZ3950Hash(qmain->getPreferredZ3950Site()).value("RecordSyntax");
		...
		!m_thread->getZ3950Results().isEmpty())
	{
								  ...
								  QMessageBox::No) == QMessageBox::Yes)
		{
			...
			if (recordSyntax == "MARC21")
				populateAfterZ3950(m_thread->getZ3950Results()[0], biblioteq_marc::MARC21);
			else
				populateAfterZ3950(m_thread->getZ3950Results()[0], biblioteq_marc::UNIMARC);
		}
	...
	{
		QMessageBox::critical(this, tr("BiblioteQ: Z39.50 Query Error"),
