							 ...
							 const int pagingType)
{
	...
	*/
	...
	if (limit == -1)
	{
		auto size = biblioteq_misc_functions::sqliteQuerySize(searchstr, m_db, __FILE__, __LINE__, this);
			...
			if (showToolTips)
			{
				...
				for (int j = 0; j < record.count(); j++)
				{
...
#endif
					{
						...
						tooltip.append(query.value(j).toString().simplified().replace("<br>", " ").simplified().trimmed());
			...
			for (int j = 0; j < record.count(); j++)
			{
						 ...
						 fieldName.endsWith("image_scaled"))
				{
					...
					if (showMainTableImages)
					{
						if (!query.isNull(j))
						{
							image.loadFromData(QByteArray::fromBase64(query.value(j).toByteArray()));
...
void biblioteq::slotPhotographSearch(void)
{
	...
	if (!photograph)
	{
		photograph = new biblioteq_photographcollection(this, "search", QModelIndex());
		photograph->search();
	}
...
void biblioteq::slotUpgradeSqliteScheme(void)
{
	...
	for (int i = 0; i < list.size(); i++)
	{
		...
		{
			errors.append(tr("<font color='red'>Error %1: %2. Statement: %3.</font>").arg(ct).arg(query.lastError().text().toLower()).arg(list.at(i)));
			ct += 1;
		}
		else
			errors.append("<font color='green'>Statement concluded correctly!</font>");
	...
	else
	{
		...
		QMessageBox::information(this,
								 ...
								 tr("The database %1 was upgraded successfully.").arg(m_db.databaseName()));
	}
