...
biblioteq_image_drop_site::biblioteq_image_drop_site(QWidget *parent) : QGraphicsView(parent)
{
...
QString biblioteq_image_drop_site::determineFormat(const QByteArray &bytes) const
{
  return biblioteq_misc_functions::imageFormatGuess(bytes);
}

QString biblioteq_image_drop_site::determineFormat(const QString &filename) const
{
...
void biblioteq_image_drop_site::dragEnterEvent(QDragEnterEvent *event)
{
  ...
  if (event)
    for (int i = 0; i < event->mimeData()->formats().size(); i++)
      ...
      {
        filename = QString(event->mimeData()->data(event->mimeData()->formats()[i])).trimmed();
        break;
      }
...
void biblioteq_image_drop_site::dropEvent(QDropEvent *event)
{
  ...
  if (!m_image.isNull())
  {
        ...
        m_image.height() > height())
    {
      ...
      if (!pixmap.isNull())
        pixmap = pixmap.scaled(0.50 * size(), Qt::KeepAspectRatio,
