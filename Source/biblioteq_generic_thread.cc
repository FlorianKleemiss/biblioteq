...
biblioteq_generic_thread::biblioteq_generic_thread(QObject *parent,
												   biblioteq *biblioteq) : QThread(parent)
{
	m_eType = "";
	m_errorStr = "";
	m_type = -1;
...
void biblioteq_generic_thread::run(void)
{
	...
	*/
	...
	case READ_GLOBAL_CONFIG_FILE:
	{
		...
		if (m_filename.trimmed().isEmpty() || !qf.open(QIODevice::ReadOnly))
		{
			if (m_filename.trimmed().isEmpty())
				...
				m_errorStr = tr("Unable to read %1. This file is required by BiblioteQ.").arg(m_filename);
	...
	case Z3950_QUERY:
	{
		...
		{
			auto value(QString("%1:%2").arg(hash.value("proxy_host")).arg(hash.value("proxy_port")));

			ZOOM_options_set(options, "proxy", value.toLatin1().constData());
		}

		ZOOM_options_set(options, "timeout", hash.value("timeout", "30").toLatin1());
		...
		ZOOM_connection_connect(zoomConnection,
								(hash.value("Address") +
								 ":" +
								 hash.value("Port"))
									.toLatin1()
									.constData(),
								0);
		zoomResultSet = ZOOM_connection_search_pqf(zoomConnection, m_z3950SearchStr.toLatin1().constData());
