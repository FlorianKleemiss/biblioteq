...
void biblioteq_files::slotExport(void)
{
  ...
  foreach (const auto &index, list)
  {
    ...
    query.prepare(QString("SELECT file FROM %1_files WHERE myoid = ?").arg(tableName.replace(' ', '_')));
...
void biblioteq_files::slotRefresh(void)
{
  ...
  if (query.exec() && query.next())
  {
    auto pages = qCeil(query.value(0).toDouble() / static_cast<double>(m_ui.pages->value()));
  ...
  query.prepare(QString("SELECT bf.file_name, "
                        "bf.description, "
                        "bf.file_digest, "
                        "LENGTH(bf.file), "
                        "b.title, "
  ...
  statusBar()->showMessage(tr("%1 Total Files").arg(m_ui.files_table->rowCount()));
