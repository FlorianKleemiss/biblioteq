...
biblioteq_sqlite_merge_databases::biblioteq_sqlite_merge_databases(biblioteq *parent) : QMainWindow(parent)
{
	m_qmain = parent;
	m_ui.setupUi(this);
...
void biblioteq_sqlite_merge_databases::slotMerge(void)
{
	...
	for (int i = 0; i < m_ui.databases->rowCount(); i++)
	{
		...
		auto progressBar = qobject_cast<QProgressBar *>(m_ui.databases->cellWidget(i, PROGRESS_COLUMN));
				...
				for (int j = 0; j < tables.size(); j++)
				{
					...
					if (query.exec(QString("SELECT * FROM %1").arg(tables.at(j))))
						while (query.next())
						{
							...
							q.prepare(QString("INSERT INTO %1 (%2) VALUES (%3)").arg(tables.at(j)).arg(str1).arg(str2));
							...
							{
								ct += 1;
								errors.append(tr("<font color='red'>Error %1: %2. "
	...
	if (dialog.result() == QDialog::Accepted)
	{
		...
		for (int i = 0; i < m_ui.databases->rowCount(); i++)
			if (m_ui.databases->cellWidget(i, SELECT_COLUMN) == pushButton)
				...
				{
					m_ui.databases->item(i, SQLITE_DATABASE_COLUMN)->setText(dialog.selectedFiles().value(0));
					m_ui.databases->item(i, SQLITE_DATABASE_COLUMN)->setToolTip(dialog.selectedFiles().value(0));
					break;
				}
