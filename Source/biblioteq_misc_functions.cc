...
QMap<QString, qint64> biblioteq_misc_functions::getItemsReservedCounts(const QSqlDatabase &db, const QString &memberid, QString &errorstr)
{
                                               ...
                                               QString &errorstr)
{
  ...
  if (itemType.toLower() == "book" ||
      itemType.toLower() == "journal" ||
      itemType.toLower() == "magazine")
    querystr = QString("SELECT EXISTS(SELECT 1 FROM %1_copy_info "
                       "WHERE copyid = ? AND item_oid = ? "
                       "AND copyid NOT IN (SELECT copyid FROM item_borrower "
                       "WHERE item_oid = ? AND type = '%2'))")
                   .arg(itemType.toLower().remove(" "))
                   .arg(itemType);
  else if (itemType.toLower() == "grey literature")
