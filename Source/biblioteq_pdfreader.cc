...
void biblioteq_pdfreader::slotPrint(void)
{
  ...
  if (dialog->exec() == QDialog::Accepted)
  {
    ...
    for (int i = start; i <= end; i++)
    {
      ...
      auto image(page->renderToImage(printer.resolution(), printer.resolution()));
...
void biblioteq_pdfreader::slotPrintPreview(void)
{
  ...
  QPrinter printer;
  QScopedPointer<QPrintPreviewDialog> dialog(new QPrintPreviewDialog(&printer, this));
...
void biblioteq_pdfreader::slotSearchNext(void)
{
  ...
  while (page < m_document->numPages())
  {
    ...
    if (m_document->page(page)->search(m_ui.find->text(),
                                       left,
                                       top,
                                       right,
                                       bottom,
                                       Poppler::Page::NextResult,
                                       m_ui.case_sensitive->isChecked() ? Poppler::Page::SearchFlag(0) : Poppler::Page::IgnoreCase))
    {
...
void biblioteq_pdfreader::slotSearchPrevious(void)
{
  ...
  while (page >= 0)
  {
    ...
    m_searchLocation = QRectF(m_document->page(page)->pageSizeF().width(),
...
void biblioteq_pdfreader::slotShowPage(int value, const QRectF &location)
{
  ...
  m_ui.page->setToolTip(tr("Page %1 of %2.").arg(value).arg(m_ui.page->maximum()));
  ...
  if (m_ui.view_size->currentIndex() == 4)
    ...
    image = page->renderToImage(scaleFactor * physicalDpiX(), scaleFactor * physicalDpiY());
  ...
  if (!location.isNull())
  {
    ...
    */
    ...
    QMatrix matrix(m_ui.view_size->currentIndex() != 4 ? scaleFactor * physicalDpiX() / 72.0 : physicalDpiX() / 72.0,
                   0,
                   0,
                   m_ui.view_size->currentIndex() != 4 ? scaleFactor * physicalDpiY() / 72.0 : physicalDpiY() / 72.0,
                   0,
                   0);
...
void biblioteq_pdfreader::slotSliderTriggerAction(int action)
{
  ...
  {
    m_ui.page->setValue(m_ui.page->value() - 2);
    m_ui.scrollArea->verticalScrollBar()->setValue(m_ui.scrollArea->verticalScrollBar()->minimum());
  }
